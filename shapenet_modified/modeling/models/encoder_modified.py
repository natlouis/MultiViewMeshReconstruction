...
###
        image_features_mean = torch.mean(image_features, 1, True).squeeze()
        # torch.Size([batch_size, 256, 8, 8])
        image_features_std = torch.std(image_features, 1, True).squeeze()
        # torch.Size([batch_size, 256, 8, 8])
        image_features_max = torch.max(image_features, 1, True)[0].squeeze()
        # torch.Size([batch_size, 256, 8, 8])
        static_image_features = [image_features_mean, image_features_max, image_features_std]
        return image_features, static_image_features
###   
